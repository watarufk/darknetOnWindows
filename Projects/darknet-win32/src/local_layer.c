...
void forward_local_layer(const local_layer l, network net)
{
    ...
    for(i = 0; i < l.batch; ++i){
        ...
        for(j = 0; j < locations; ++j){
            ...
            SGEMM("N", "N", &m, &n, &k, &alpha, &a, &k, &b, &locations, &beta, &c, &locations);
#else
    ...
    for(i = 0; i < l.batch; ++i){
...
#ifdef INTEL_MKL
            float alpha = 1.0f, beta = 1.0f;
            SGEMM("N", "T", &m, &n, &k, &alpha, &a, &locations, &b, &locations, &beta, &c, &n);
#else
        ...
        if(net.delta){
            for(j = 0; j < locations; ++j){ 
                ...
                SGEMM("T", "N", &m, &n, &k, &alpha, &a, &m, &b, &locations, &beta, &c, &locations);
#else
